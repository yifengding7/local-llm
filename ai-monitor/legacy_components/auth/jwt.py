...
class JWTHandler:
    ...
    def create_refresh_token(self, data: Dict[str, Any], expires_delta: timedelta = timedelta(days=7)) -> str:
        """创建刷新令牌"""
        to_encode = data.copy()
        expire = datetime.utcnow() + expires_delta
    ...
    def refresh_access_token(self, refresh_token: str, access_expires: timedelta = timedelta(minutes=30)) -> str:
        """刷新访问令牌"""
        payload = jwt.decode(refresh_token, self.secret_key, algorithms=[self.algorithm])
        if payload.get("type") != "refresh":
            raise ValueError("Invalid token type")
        user_id = payload.get("sub")
        return self.create_access_token({"sub": user_id}, access_expires)
