...
AI_MONITOR_HOME="$PROJECT_HOME/ai-monitor-v2"
...
start_api_dev() {
    ...
    if ! lsof -i :8000 > /dev/null 2>&1; then
        echo "启动中..."
        poetry run uvicorn ai_monitor.api.main:app --reload --host 0.0.0.0 --port 8000 > "$LOGS_DIR/api-dev.log" 2>&1 &
...
launch_gui() {
    ...
    poetry run python ai_monitor/gui/main.py
}

# 启动Web UI
launch_ui() {
    echo -e "${YELLOW}启动Web控制面板...${NC}"
    poetry run python ai_monitor/gui/control_panel_server.py > "$LOGS_DIR/control-panel.log" 2>&1 &
    ...
    while true; do
        ...
        case $choice in
            1)
                echo -e "\n${YELLOW}文件路径:${NC} "
                read file_path
                poetry run python ai_monitor/scaffold/claude_helper.py analyze "$file_path"
                echo -e "\n按任意键继续..."
                read -n 1
                ;;
            2)
                poetry run python ai_monitor/scaffold/claude_code_mcp_tool.py
                echo -e "\n按任意键继续..."
                read -n 1
                ;;
            0)
...
handle_command_mode() {
    case "$1" in
        ...
        launch_ui)
            ...
            poetry run python ai_monitor/gui/control_panel_server.py &
            sleep 2
            open "http://localhost:5005" 2>/dev/null || xdg-open "http://localhost:5005"
            ;;
        gui)
